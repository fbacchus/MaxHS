...
void Internal::flush_all_occs_and_watches() {
  if (occurring())
    for (auto idx : vars) flush_occs(idx), flush_occs(-idx);

  if (watching()) {
    Watches tmp;
    for (auto idx : vars) flush_watches(idx, tmp), flush_watches(-idx, tmp);
  }
}
...
void Internal::copy_non_garbage_clauses() {
  ...
  PHASE("collect", stats.collections,
        ...
        percent(moved_bytes, collected_bytes + moved_bytes), moved_clauses);
  ...
  if (opts.arenacompact)
    for (const auto& c : clauses)
      if (!c->collect() && arena.contains(c)) copy_clause(c);
  ...
  if (opts.arenatype == 1 || !watching()) {
    ...
    for (const auto& c : clauses)
      if (!c->moved && !c->collect()) copy_clause(c);
    ...
    for (int sign = -1; sign <= 1; sign += 2)
      for (auto idx : vars)
        for (const auto& w : watches(sign * likely_phase(idx)))
          if (!w.clause->moved && !w.clause->collect()) copy_clause(w.clause);
  ...
  //
  for (const auto& c : clauses)
    if (!c->collect() && !c->moved) copy_clause(c);
  ...
  if (opts.arenasort) rsort(clauses.begin(), clauses.end(), pointer_rank());
  ...
  PHASE("collect", stats.collections,
        "collected %zd bytes %.0f%% of %zd garbage clauses", collected_bytes,

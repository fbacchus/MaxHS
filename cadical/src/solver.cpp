...
#define STATE(S)                                                              \
  do {                                                                        \
    ...
    if (_state == S) break;                                                   \
    _state = S;                                                               \
    LOG("API enters state %s" #S "%s", tout.emph_code(), tout.normal_code()); \
  } while (0)
...
void Solver::transition_to_unknown_state() {
  if (state() == CONFIGURING) {
    ...
    LOG("API leaves state %sSATISFIED%s", tout.emph_code(), tout.normal_code());
...
static void log_api_call(Internal* internal, const char* name, int arg,
                         const char* suffix) {
  Logger::log(internal, "API call %s'%s (%d)'%s %s", tout.api_code(), name, arg,
              tout.log_code(), suffix);
}

static void log_api_call(Internal* internal, const char* name, const char* arg,
                         const char* suffix) {
  Logger::log(internal, "API call %s'%s (\"%s\")'%s %s", tout.api_code(), name,
              arg, tout.log_code(), suffix);
}

static void log_api_call(Internal* internal, const char* name, const char* a1,
                         int a2, const char* s) {
  Logger::log(internal, "API call %s'%s (\"%s\", %d)'%s %s", tout.api_code(),
              name, a1, a2, tout.log_code(), s);
}
...
static void log_api_call_begin(Internal* internal, const char* name, int arg) {
  Logger::log_empty_line(internal);
...
static void log_api_call_end(Internal* internal, const char* name, int lit) {
...
static void log_api_call_returns(Internal* internal, const char* name, int lit,
                                 int res) {
  char fmt[32];
  sprintf(fmt, "returns '%d'", res);
...
static void log_api_call_returns(Internal* internal, const char* name,
                                 const char* arg, bool res) {
  log_api_call(internal, name, arg, res ? "returns 'true'" : "returns 'false'");
}

static void log_api_call_returns(Internal* internal, const char* name, int lit,
                                 bool res) {
  log_api_call(internal, name, lit, res ? "returns 'true'" : "returns 'false'");
}
...
static void log_api_call_returns(Internal* internal, const char* name,
                                 const char* arg1, int arg2, const char* res) {
...
bool Solver::is_valid_option(const char* name) { return Options::has(name); }
...
bool Solver::set(const char* arg, int val) {
  TRACE("set", arg, val);
  REQUIRE_VALID_STATE();
  if (strcmp(arg, "log") && strcmp(arg, "quiet") && strcmp(arg, "report") &&
      strcmp(arg, "verbose") && strcmp(arg, "elimrounds") && strcmp(arg, "vivify")) {
...
int Solver::simplify(int rounds) {
  ...
  REQUIRE_READY_STATE();
  REQUIRE(rounds >= 0, "negative number of simplification rounds '%d'", rounds);
...
bool Solver::find_up_implicants(vector<int> antecedents,
                                vector<int>& implicants) {
  TRACE("find_up_implicants");
  REQUIRE_READY_STATE();
  transition_to_unknown_state();
  auto res{external->find_up_implicants(antecedents, implicants)};
  external->reset_assumptions();
  transition_to_unknown_state();
  LOG_API_CALL_RETURNS("find_up_implicants", res);
  return res;
}

bool Solver::root_level_propagate() {
  TRACE("root_level_propagate");
  REQUIRE_READY_STATE();
  auto res = internal->root_level_propagate();
  LOG_API_CALL_RETURNS("root_level_propagate", res);
  return res;
}

int Solver::val(int lit) const {
  TRACE("val", lit);
  REQUIRE_VALID_STATE();
  REQUIRE_VALID_LIT(lit);
...
int64_t Solver::n_propagations() const {
  TRACE("n_propagations");
  REQUIRE_VALID_STATE();
  int64_t res = internal->n_propagations();
  LOG_API_CALL_RETURNS("n_propagations", res);
  return res;
}

int64_t Solver::n_decisions() const {
  TRACE("n_decisions");
  REQUIRE_VALID_STATE();
  int64_t res = internal->n_decisions();
  LOG_API_CALL_RETURNS("n_decisions", res);
  return res;
}

int64_t Solver::n_conflicts() const {
  TRACE("n_conflicts");
  REQUIRE_VALID_STATE();
  int64_t res = internal->n_conflicts();
  LOG_API_CALL_RETURNS("n_conflicts", res);
  return res;
}

int64_t Solver::n_eliminated() const {
  TRACE("n_eliminated");
  REQUIRE_VALID_STATE();
  int64_t res = internal->n_eliminated();
  LOG_API_CALL_RETURNS("n_eliminated", res);
  return res;
}

size_t Solver::n_internal_fixed() const {
  TRACE("n_internal_fixed");
  REQUIRE_VALID_STATE();
  auto res = external->units.size();
  LOG_API_CALL_RETURNS("n_internal_fixed", static_cast<int64_t>(res));
  return res;
}

const vector<int>& Solver::solver_units() const {
  TRACE("solver_units");
  REQUIRE_VALID_STATE();
  auto res [[maybe_unused]] = external->units.size();
  LOG_API_CALL_RETURNS("solver_units", static_cast<int64_t>(res));
  return external->units;
}

int64_t Solver::n_substituted() const {
  TRACE("n_substituted");
  REQUIRE_VALID_STATE();
  int64_t res = internal->n_substituted();
  LOG_API_CALL_RETURNS("n_substitued", res);
  return res;
}
...
void Solver::melt(int lit) {
  ...
  REQUIRE_VALID_STATE();
  REQUIRE_VALID_LIT(lit);
  REQUIRE(external->frozen(lit), "can not melt completely melted literal '%d'",
          lit);
  external->melt(lit);
...
const char* Solver::read_solution(const char* path) {
  ...
  File* file = File::read(internal, path);
  if (!file)
    return internal->error_message.init("failed to read solution file '%s'",
                                        path);
...
size_t Solver::n_clauses() const {
  LOG_API_CALL_BEGIN("n_clauses");
  REQUIRE_VALID_STATE();
  auto res = internal->n_clauses();
  LOG_API_CALL_RETURNS("n_clauses", static_cast<int64_t>(res));
  return res;
}

bool Solver::get_ith_clause(vector<int>& cls, size_t i) const {
  LOG_API_CALL_BEGIN("get_ith_clauses");
  REQUIRE_VALID_STATE();
  auto res = internal->get_ith_clause(cls, i);
  LOG_API_CALL_RETURNS("get_num_clauses", static_cast<int64_t>(i));
  return res;
}
...
const char* Solver::write_dimacs(const char* path, int min_max_var) {
  ...
  if (file) {
    ...
    if (!traverse_clauses(writer))
      res = internal->error_message.init("writing to DIMACS file '%s' failed",
                                         path);
    delete file;
  } else
  ...
  if (!res) {
    ...
    MSG("wrote %" PRId64 " clauses in %.2f seconds %s time", counter.clauses,
...
const char* Solver::write_extension(const char* path) {
  ...
  if (!res) {
    const double end = internal->time();
    MSG("wrote %" PRId64 " witnesses in %.2f seconds %s time", writer.witnesses,
        end - start, internal->opts.realtime ? "real" : "process");
  }
#endif
...
struct ClauseCopier : public ClauseIterator {
 ...
 public:
  ...
  bool clause(const vector<int>& c) {
    for (const auto& lit : c) dst.add(lit);
    dst.add(0);
    return true;
  }
};

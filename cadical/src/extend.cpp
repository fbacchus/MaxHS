...
void External::push_clause_literal_on_extension_stack(int ilit) {
  ...
  LOG("pushing clause literal %d on extension stack (internal %d)", elit, ilit);
}
...
void External::push_clause_on_extension_stack(Clause* c) {
  ...
  for (const auto& lit : *c) push_clause_literal_on_extension_stack(lit);
}
...
void External::push_binary_clause_on_extension_stack(int pivot, int other) {
  internal->stats.weakened++;
...
void External::extend() {
  ...
  PHASE("extend", internal->stats.extensions,
        "mapping internal %d assignments to %d assignments", internal->max_var,
        max_var);
  ...
  for (unsigned i = 1; i <= (unsigned)max_var; i++) {
    ...
    if (!ilit) continue;
    while (i >= vals.size()) vals.push_back(false);
  ...
  PHASE("extend", internal->stats.extensions,
        "extending through extension stack of size %zd", extension.size());
    ...
    if (satisfied)
      ...
      while ((lit = *--i)) {
        ...
        if (tmp < 0) {
          ...
          assert(lit != INT_MIN);
          int idx = abs(lit);
          while ((size_t)idx >= vals.size()) vals.push_back(false);
          vals[idx] = !vals[idx];
...
bool External::traverse_witnesses_backward(WitnessIterator& it) {
  ...
  while (i != begin) {
    int lit;
    while ((lit = *--i)) clause.push_back(lit);
    while ((lit = *--i)) witness.push_back(lit);
    reverse(clause.begin(), clause.end());
    reverse(witness.begin(), witness.end());
    if (!it.witness(clause, witness)) return false;
    clause.clear();
    witness.clear();
  }
  return true;
...
bool External::traverse_witnesses_forward(WitnessIterator& it) {
  ...
  if (i != end) {
    ...
    do {
      ...
      assert(!lit);
      assert(i != end);
      while (i != end && (lit = *i++)) clause.push_back(lit);
      if (!it.witness(clause, witness)) return false;
      clause.clear();
      witness.clear();
    } while (i != end);

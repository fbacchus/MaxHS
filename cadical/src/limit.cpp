...
void Internal::limit_propagations(int64_t l) {
  if (l < 0 && inc.propagations < 0) {
    LOG("keeping unbounded proapgation limit");
  } else if (l < 0) {
    LOG("reset propagation limit to be unbounded");
    inc.propagations = -1;
  } else {
    inc.propagations = l;
    LOG("new propagation limit of %d progation", l);
  }
}
...
bool Internal::is_valid_limit(const char* name) {
  ...
  if (!strcmp(name, "propagations")) return true;
...
bool Internal::limit(const char* name, int64_t l) {
  ...
  if (!strcmp(name, "terminate"))
    limit_terminate(l);
  else if (!strcmp(name, "propagations"))
    limit_propagations(l);

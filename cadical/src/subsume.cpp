...
bool Internal::subsuming() {
  ...
  //
  if (opts.reduce && stats.conflicts != last.reduce.conflicts) return false;
...
bool Internal::subsume_round() {
  ...
  for (auto c : clauses) {
    ...
    for (const auto& lit : *c) noccs(lit)++;
  }
  ...
  if (!left_over_from_last_subsumption_round)
    for (auto cs : schedule)
      if (cs.clause->size > 2) cs.clause->subsume = true;
  ...
  for (const auto& c : shrunken) mark_added(c);

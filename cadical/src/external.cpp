...
void External::init(int new_max_var) {
  ...
  for (eidx = max_var + 1u; eidx <= (unsigned)new_max_var; eidx++, iidx++) {
  ...
  if (internal->opts.checkfrozen)
    while (new_max_var >= (int64_t)moltentab.size()) moltentab.push_back(false);
...
int External::internalize(int elit) {
  ...
  if (elit) {
    ...
    if (internal->opts.checkfrozen) {
      ...
      if (moltentab[eidx]) FATAL("can not reuse molten literal %d", eidx);
    }
...
void External::check_satisfiable() {
  ...
  if (internal->opts.checkwitness) check_assignment(&External::ival);
...
bool External::find_up_implicants(vector<int> antecedents,
                                  vector<int>& implicants) {
  reset_extended();
  for (auto& elit : antecedents) assume(elit);
  update_molten_literals();
  auto res = internal->find_up_implicants(implicants);
  for (auto& ilit : implicants) ilit = internal->externalize(ilit);
  reset_limits();
  return res;
}
...
void External::freeze(int elit) {
  ...
  while (eidx >= frozentab.size()) frozentab.push_back(0);
  unsigned& ref = frozentab[eidx];
  if (ref < UINT_MAX) {
    ref++;
    LOG("external variable %d frozen once and now frozen %u times", eidx, ref);
  } else
...
void External::melt(int elit) {
  ...
  if (ref < UINT_MAX) {
    if (!--ref)
      ...
      LOG("external variable %d melted once but remains frozen %u times", eidx,
          ref);
  } else
...
void External::check_assignment(int (External::*a)(int) const) {
  ...
  for (; i != end; i++) {
    ...
    if (!lit) {
      if (!satisfied) {
        ...
        for (auto j = start; j != i; j++) fprintf(stderr, "%d ", *j);
        fputc('0', stderr);
        fatal_message_end();
      }
      satisfied = false;
  ...
  VERBOSE(1, "satisfying assignment checked on %" PRId64 " clauses", count);
}
...
void External::check_assumptions_satisfied() {
  ...
  VERBOSE(1, "checked that %zd assumptions are satisfied", assumptions.size());
}
...
#endif
  for (const auto& lit : original) checker->add(lit);
  for (const auto& lit : assumptions) {
...
bool External::traverse_all_frozen_units_as_clauses(ClauseIterator& it) {
  ...
  for (auto idx : vars) {
    ...
    if (!it.clause(clause)) return false;
    clause.clear();
  }
...
bool External::traverse_all_non_frozen_units_as_witnesses(WitnessIterator& it) {
  if (internal->unsat) return true;
  ...
  for (auto idx : vars) {
    ...
    if (!it.witness(clause_and_witness, clause_and_witness)) return false;

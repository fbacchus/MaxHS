...
long Internal::condition_round(long delta) {
  ...
  for (const auto& c : clauses) {
    ...
    //
    for (const_literal_iterator l = c->begin(); !satisfied && l != c->end();
         l++) {
      const int lit = *l;
  ...
  PHASE("condition", stats.conditionings, "found %zd candidate clauses",
        candidates.size());
  PHASE("condition", stats.conditionings, "watching %ld literals and clauses",
        watched);
  ...
  for (size_t i = 0; i < trail.size(); i++)
    if (is_autarky_literal(trail[i])) LOG("initial autarky %d", trail[i]);
#endif
  ...
  for (const auto& lit : trail)
    if (fixed(lit)) condition_unassign(lit);
  ...
  if (conditioned && unconditioned) {
    ...
    PHASE("condition", stats.conditionings, "all %zd candidates tried before",
          conditioned);
  } else {
    assert(!conditioned);
  ...
  for (const auto& c : candidates) {
    ...
    if (terminated_or_limit_hit) {
      PHASE("condition", stats.conditionings,
            "%zd candidates %.0f%% not tried after %ld propagations", untried,
            percent(untried, candidates.size()), props);
      break;
    }
    untried--;
    ...
    while (watched_autarky_literal && stats.condprops < limit &&
      ...
      while (watched_autarky_literal && stats.condprops < limit &&
        ...
        for (; watched_autarky_literal && j != os.end(); j++) {
          ...
          for (const_literal_iterator l = d->begin(); l != d->end(); l++) {
            const int lit = *l;
            const signed char tmp = val(lit);
          ...
          for (const_literal_iterator l = d->begin();
            ...
            if (replacement) {
              LOG(c, "watching autarky %d instead %d in candidate", replacement,
        ...
        if (i < j) {
          while (j != os.end()) *i++ = *j++;
          LOG("flushed %zd occurrences of %d", os.end() - i, unassigned_lit);
  ...
  PHASE("condition", stats.conditionings, "globally blocked %ld clauses %.0f%%",
        blocked, percent(blocked, candidates.size()));

  // Unmark initial conditional variables.
  //
  for (const auto& lit : conditional) unmark_as_conditional_literal(lit);
  ...
  LOG("unassigned %d additionally assigned literals", additionally_unassigned);
...
#ifndef NDEBUG
  for (const auto& lit : trail) assert(!marked(lit));
#endif
...
void Internal::condition(bool update_limits) {
  ...
  PHASE("condition", stats.conditionings,
        "next limit at %ld after %ld conflicts", lim.condition, delta);
}

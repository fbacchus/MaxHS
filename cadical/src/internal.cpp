...
Internal::~Internal() {
  for (const auto& c : clauses) delete_clause(c);
...
void Internal::init_vars(int new_max_var) {
  ...
  if (level) backtrack();
  LOG("initializing %d internal variables from %d to %d", new_max_var - max_var,
      max_var + 1, new_max_var);
...
void Internal::init_preprocessing_limits() {
  ...
  (void)mode;
  LOG("%s elim limit %" PRId64 " after %" PRId64 " conflicts", mode, lim.elim,
      lim.elim - stats.conflicts);
  ...
  if (!incremental) {
    ...
    LOG("initial compact limit %" PRId64 " increment %" PRId64 "", lim.compact,
        lim.compact - stats.conflicts);
  }
  ...
  (void)mode;
  LOG("%s probe limit %" PRId64 " after %" PRId64 " conflicts", mode, lim.probe,
      lim.probe - stats.conflicts);
  ...
  LOG("%s condition limit %ld increment %ld", mode, lim.condition,
      lim.condition - stats.conflicts);
...
void Internal::init_search_limits() {
  ...
  LOG("new rephase limit %" PRId64 " after %" PRId64 " conflicts", lim.rephase,
  ...
  if (opts.stabilize && opts.reluctant) {
    LOG("new restart reluctant doubling sequence period %d", opts.reluctant);
  ...
  if (inc.decisions < 0) {
    ...
    LOG("decision limit after %" PRId64 " decisions at %" PRId64 " decisions",
  ...
  if (inc.propagations < 0) {
    ...
    lim.propagations = stats.total_propagations() + inc.propagations;
    LOG("propagation limit after %" PRId64 " propagations at %" PRId64
        " propagations",
        inc.propagations, lim.propagations);
  }
...
/*------------------------------------------------------------------------*/
bool Internal::find_up_implicants(vector<int>& implicants) {
  START(find_up_implicants);
  assert(clause.empty());
  implicants.clear();
  bool ok{false};
  int res = already_solved();
  auto save_opt = opts.restoreall;
  opts.restoreall = 2;
  if (!res) res = restore_clauses();
  opts.restoreall = save_opt;
  if (!res) {
    ...
    auto cur_trail_size = trail.size();
    ok = propagate();
    if (ok) {
      for (auto i = cur_trail_size; i < trail.size(); i++)
        implicants.push_back(trail[i]);
    }
    conflict = 0;
  }
  STOP(find_up_implicants);
  return ok;
}

bool Internal::root_level_propagate() {
  auto res = already_solved();
...
void Internal::dump(Clause* c) {
  for (const auto& lit : *c) printf("%d ", lit);
  printf("0\n");
}
...
void Internal::dump() {
  ...
  for (const auto& c : clauses)
    if (!c->garbage) dump(c);
  for (const auto& lit : assumptions) printf("%d 0\n", lit);
  fflush(stdout);
}
...
bool Internal::traverse_clauses(ClauseIterator& it) {
  ...
  for (const auto& c : clauses) {
    ...
    if (!satisfied && !it.clause(eclause)) return false;
    eclause.clear();
  }
  return true;
}
...
/*---------------------------------------------------------------------------*/
// Place i-th clause in passed vector.
// Return true if the clause is redundant.
// Set returned vector to empty if i-th clause has been marked as garbage
// OR formula is UNSAT OR clause is satisfied by a forced unit.

bool Internal::get_ith_clause(vector<int>& cls, size_t i) const {
  cls.clear();
  if (unsat || i >= clauses.size() || clauses[i]->garbage) return false;
  bool redundant = clauses[i]->redundant;
  for (auto ilit : *(clauses[i])) {
    auto tmp = fixed(ilit);
    if (tmp > 0) {
      cls.clear();
      return false;

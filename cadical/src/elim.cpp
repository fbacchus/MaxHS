...
void Internal::elim_update_added_clause(Eliminator& eliminator, Clause* c) {
  assert(!c->redundant);
...
void Internal::elim_update_removed_clause(Eliminator& eliminator, Clause* c,
                                          int except) {
  assert(!c->redundant);
  for (const auto& lit : *c) {
...
bool Internal::resolve_clauses(Eliminator& eliminator, Clause* c, int pivot,
                               Clause* d) {
  assert(!c->redundant);
  assert(!d->redundant);
...
bool Internal::elim_resolvents_are_bounded(Eliminator& eliminator, int pivot) {
  ...
  LOG("need %" PRId64 " <= %" PRId64 " non-tautological resolvents", resolvents,
      bound);
...
inline void Internal::elim_add_resolvents(Eliminator& eliminator, int pivot) {
  const bool substitute = !eliminator.gates.empty();
  if (substitute) {
...
void Internal::try_to_eliminate_variable(Eliminator& eliminator, int pivot) {
  ...
  LOG("pivot %d occurs positively %" PRId64 " times and negatively %" PRId64
      " times",
      pivot, pos, neg);
...
int Internal::elim_round(bool& completed) {
  ...
  if (opts.elimlimited) {
    ...
    PHASE("elim-round", stats.elimrounds, "limit of %" PRId64 " resolutions",
          delta);
  ...
  PHASE("elim-round", stats.elimrounds,
        "scheduled %" PRId64 " variables %.0f%% for elimination", scheduled,
        percent(scheduled, active()));
  ...
  for (const auto& c : clauses)
    if (!c->garbage && !c->redundant)
      for (const auto& lit : *c)
        if (active(lit)) occs(lit).push_back(c);
  ...
  PHASE("elim-round", stats.elimrounds,
        "tried to eliminate %" PRId64 " variables %.0f%% (%zd remain)", tried,
        percent(tried, scheduled), schedule.size());
...
void Internal::increase_elimination_bound() {
  ...
  PHASE("elim-phase", stats.elimphases, "new elimination bound %" PRId64 "",
        lim.elimbound);

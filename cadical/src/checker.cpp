...
CheckerClause* Checker::new_clause() {
  ...
  for (const auto& lit : simplified) *p++ = lit;
  num_clauses++;
...
bool Checker::clause_satisfied(CheckerClause* c) {
  for (unsigned i = 0; i < c->size; i++)
    if (val(c->literals[i]) > 0) return true;
  return false;
}
...
void Checker::collect_garbage_clauses() {
  ...
  LOG("CHECKER collecting %zu garbage clauses %.0f%%", num_garbage,
...
void Checker::import_clause(const vector<int>& c) {
  for (const auto& lit : c) import_literal(lit);
}
...
CheckerClause** Checker::find() {
  ...
  for (res = clauses + h; (c = *res); res = &c->next) {
    if (c->hash == hash && c->size == size) {
      ...
      for (unsigned i = 0; found && i != size; i++) found = mark(literals[i]);
      if (found) break;
    }
    stats.collisions++;
  }
  ...
  while (res && next_to_propagate < trail.size()) {
    ...
    for (; res && i != end; i++) {
      ...
      if (size == 2) {  // not precise since
        ...
        for (k = 2; k < size; k++)
          if ((replacement_val = val(replacement = lits[k])) >= 0) break;
...
bool Checker::check() {
  ...
  for (const auto& lit : simplified) assume(-lit);
  bool res = !propagate();
...
void Checker::add_derived_clause(const vector<int>& c) {
  ...
  if (tautological())
    ...
    for (const auto& lit : unsimplified) fprintf(stderr, "%d ", lit);
    fputc('0', stderr);
    fatal_message_end();
  } else
    add_clause("derived");
...
void Checker::dump() {
  ...
  for (uint64_t i = 0; i < size_clauses; i++)
    for (CheckerClause* c = clauses[i]; c; c = c->next)
      for (unsigned i = 0; i < c->size; i++)
        if (abs(c->literals[i]) > max_var) max_var = abs(c->literals[i]);
  printf("p cnf %d %" PRIu64 "\n", max_var, num_clauses);
  for (uint64_t i = 0; i < size_clauses; i++)
    for (CheckerClause* c = clauses[i]; c; c = c->next) {
      for (unsigned i = 0; i < c->size; i++) printf("%d ", c->literals[i]);
      printf("0\n");
    }
}

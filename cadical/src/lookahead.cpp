...
void Internal::lookahead_generate_probes() {
  ...
  PHASE("probe-round", stats.probingrounds,
        "scheduled %" PRId64 " literals %.0f%%", probes.size(),
...
int Internal::lookahead_probing() {
  ...
  while (!unsat && !terminating_asked() && (probe = lookahead_next_probe())) {
    ...
    if (max_hbrs < hbrs ||
        (max_hbrs == hbrs && internal->bumped(probe) > internal->bumped(res))) {
      res = probe;
      max_hbrs = hbrs;
    }
  }
...
CubesWithStatus Internal::generate_cubes(int depth) {
  if (!active()) return CubesWithStatus();
  ...
  for (int i = 0; i < depth; ++i) {
    ...
    for (size_t j = 0; j < cubes2.size(); ++j) {
      reset_assumptions();
      for (auto lit : cubes2[j]) assume(lit);
      restore_clauses();
    ...
    if (terminating_asked()) break;
  }

  assert(std::for_each(begin(cubes), end(cubes), [](std::vector<int> cube) {
    return non_tautological_cube(cube);
  }));
  reset_assumptions();

  for (auto lit : current_assumptions) assume(lit);

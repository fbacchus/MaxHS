...
void Internal::mark_removed(Clause* c, int except) {
  ...
  for (const auto& lit : *c)
    if (lit != except) mark_removed(lit);
}
...
void Internal::mark_added(Clause* c) {
  ...
  for (const auto& lit : *c) mark_added(lit, c->size, c->redundant);
}
...
#ifndef NDEBUG
  for (int i = c->size; i < new_size; i++) c->literals[i] = 0;
#endif
...
void Internal::delete_clause(Clause* c) {
  ...
  if (c->garbage) {
    ...
    //
    if (proof && c->size == 2) proof->delete_clause(c);
  }
  deallocate_clause(c);
}
...
void Internal::add_new_original_clause() {
  ...
  if (unsat) {
    ...
    for (const auto& lit : original) unmark(lit);
  }
  if (skip) {

...
class Solver {
 public:
  ...
  // Add new function.
  // Given a set of literals us unit propagation to find all implicants;
  // Return false if antecedents are inconsistent (generate a conflict).
  //        in this case the vector implicants is invalid.
  bool find_up_implicants(vector<int> antecedents, vector<int>& implicants);

  // Find unit prop implicants of added clauses by backtracking to level 0 and
  // doing unit prop. Return false unsat found.
  bool root_level_propagate();
  ...
  int64_t n_propagations() const; // Number of propagations made by sat solver so far
  int64_t n_decisions() const;    // Number of decisions made by sat solver so far
  int64_t n_conflicts() const;    // Number of conflicts found so far
  int64_t n_eliminated() const;   // Number of variables eliminated so far
  size_t  n_internal_fixed() const;   // Number of variables eliminated so far
  int64_t n_substituted() const;   // Number of variables eliminated so far

  const vector<int>& solver_units() const; //return ref to units found by solver
  ...
  bool get_ith_clause(vector<int>& cls, size_t i) const;
  ...
  const char* read_dimacs(FILE* file, const char* name, int& vars, int strict,

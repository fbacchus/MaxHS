...
struct Internal {
  ...
  int64_t n_propagations() const { return stats.total_propagations(); }
  int64_t n_decisions() const { return stats.decisions; }
  int64_t n_conflicts() const { return stats.conflicts; }
  int64_t n_eliminated() const { return stats.all.eliminated; }
  int64_t n_fixed() const { return stats.all.fixed; }
  int64_t n_substituted() const { return stats.all.substituted; }  
  ...
  void unmark(vector<int>& lits) {
    for (const auto& lit : lits) unmark(lit);
  }
  ...
  void limit_propagations(int64_t); //Force propagation limit
  ...
  bool find_up_implicants(vector<int>& implicants);
  ...
  void melt(int lit) {
    ...
    if (ref < UINT_MAX) {
      if (!--ref) {
        ...
        LOG("variable %d melted once but remains frozen %u times", lit, ref);
    } else
      LOG("variable %d remains frozen forever", idx);
  }
  ...
  size_t n_clauses() const { return clauses.size(); }
...
inline bool Internal::search_limits_hit() {
  ...
  if (lim.propagations >= 0 && stats.total_propagations() >= lim.propagations) {
    LOG("propagation limit %" PRId64 " reached", lim.propagations);
    return true;
  }

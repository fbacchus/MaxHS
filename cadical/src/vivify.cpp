  ...
  if (level) {
    ...
    for (const auto& lit : sorted) {
      ...
      if (-lit == decision) {
        ...
        LOG("literal %d does not match decision %d at decision level %d", lit,
            decision, l);
        backtrack(l - 1);
        break;
      }
...
void Internal::vivify_round(bool redundant_mode, int64_t propagation_limit) {
  ...
  for (const auto& c : clauses) {
    ...
    for (const auto lit : *c) noccs(lit) += score;
  }
  ...
  if (!unsat) {
    ...
    for (const auto& c : vivifier.schedule)
      if (c->vivify) still_need_to_be_vivified++;
    ...
    if (still_need_to_be_vivified)
      ...
      for (const auto& c : vivifier.schedule) c->vivify = true;
    }
  ...
  PHASE("vivify", stats.vivifications,
        "checked %" PRId64 " clauses %.02f%% of %" PRId64 " scheduled", checked,
        percent(checked, scheduled), scheduled);
  if (units)
    PHASE("vivify", stats.vivifications,
